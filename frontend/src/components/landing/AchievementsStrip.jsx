...
const AchievementsStrip = () => {
    return (
        <div className="relative z-30 -mt-12 mb-0">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                    {features.map((f, i) => (
                        <a
                            key={i}
                            href={f.href}
                            className={`${f.color} rounded-xl p-4 hover:shadow-xl transition-all hover:-translate-y-1 duration-300 group cursor-pointer block`}
                        >
                            <div className="mb-3 opacity-80 group-hover:opacity-100 transition-opacity">
                                {f.icon}
                            </div>
                            <h4 className="text-sm font-bold leading-tight mb-1">{f.title}</h4>
                            <p className="text-[11px] leading-relaxed opacity-75">{f.desc}</p>
                        </a>
                    ))}
                </div>
            </div>
